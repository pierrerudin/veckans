...
class AzureDataLakeConnector:
    ...
    def generate_cache_path(self, container_name: str, path_in_container: str, columns, filter_expression, base_dir: str = "data/cache"):
        ...
        # Convert filter_expression to a serializable format
        if filter_expression is not None:
            # Use pickle to serialize the full expression including all values in is_in conditions
            # This captures the complete expression, not just the truncated string representation
            try:
                import pickle
                filter_str = pickle.dumps(filter_expression).hex()
            except:
    ...
    def read_delta_table(self,
        ...
        if not all([azure_client_id, azure_client_secret, azure_tenant_id, storage_account]):
            missing = []
            ...
            if not storage_account: missing.append("storage_account")
