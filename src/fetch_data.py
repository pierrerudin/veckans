...
def _fetch_orders_by_items(datalake_connector: AzureDataLakeConnector, item_skeys: list):
    """
    Fetch complete order history for specific items (for training models).
    
    Parameters:
        datalake_connector: The connector to interact with the Data Lake
        item_skeys: List of item Skeys to fetch order history for
    
    Returns:
        DataFrame with complete order history for those items
    """
    logging.info(f"Fetching historical order data for {len(item_skeys)} unique items from Azure Data Lake...")
    
    filter_expression = (
        (ds.field("itemSkey").isin(item_skeys)) &
        (ds.field("orderCategoryId") == 1) &
        ds.field("orderCancelledDatetime").is_null() &
        (ds.field("cancelFlag") == "N") &
        (ds.field("deliveryDate") >= date(2022, 1, 1))
    )
    ...
    query = {
        ...
        "path_in_container": "dm/out/fact_order",
        "batch_size": 5_000_000,
        "columns": ['orderDate', 'deliveryDate', 'itemId', 'itemSkey', 'salesCampignId', 'salesQuantity'],
        "filter_expression": filter_expression,
        "use_cache": True,
        "storage_format": "delta"
    }
...
def fetch_data(simulate_item_ids: list[str] = None):
    ...
    cluster_ids = [cid for cid in cluster_ids if not _is_empty(cid)]
    ...
    # Now we have all items we are interested
    # GREAT!!! One more item fetch though...
    # All items might have one or more copies with a leading M, S or K in
    # So itemId 528208 might also exist as M528208, S528208 or K528208
    # We need to fetch those as well to have a complete item list for clustering
    leading_prefixes = ['M', 'S', 'K']
    additional_item_ids = []
    for item_id in df_items['itemId'].unique():
        for prefix in leading_prefixes:
            if not str(item_id).startswith(tuple(leading_prefixes)):
                additional_item_ids.append(f"{prefix}{item_id}")
    df_additional_items = _fetch_items_by_ids(adl_connector, additional_item_ids)
    logging.info(f"Fetched {len(df_additional_items)} additional items with leading prefixes.")
    df_items = pd.concat([df_items, df_additional_items], ignore_index=True)

    # Now we need to add a column to df_items that is the itemId without leading M, S, K
    df_items['baseItemId'] = df_items['itemId'].apply(lambda x: x[1:] if str(x).startswith(tuple(leading_prefixes)) else x)
   
   # And make sure that all items with the same baseItemId have the same cluster1To1Id
    cluster_mapping = df_items.drop_duplicates(subset='baseItemId').set_index('baseItemId')['cluster1To1Id'].to_dict()
    df_items['cluster1To1Id'] = df_items['baseItemId'].map(cluster_mapping)
    
    # Remove duplicates
    df_items = df_items.drop_duplicates(subset='itemSkey', keep='first')
    logging.info(f"Total unique items after fetching clusters: {len(df_items)}")
    
    # Fill in missing cluster IDs by assigning itemId as cluster1To1Id
    missing_cluster_items = df_items[(df_items['cluster1To1Id'] == '') | (df_items['cluster1To1Id'].isna())]['itemId'].tolist()
    for item_id in missing_cluster_items:
        df_items.loc[df_items['itemId'] == item_id, 'cluster1To1Id'] = item_id

    logging.info(f"Assigned individual clusters to {len(missing_cluster_items)} items without cluster assignments.")

    # Step 5: Fetch orders for all items
    all_item_skeys = df_items['itemSkey'].unique().tolist()
    df_orders = _fetch_orders_by_items(adl_connector, all_item_skeys)
...
if __name__ == "__main__":
    item_ids = ['528208']
    df_items, df_orders, df_weekly_deals = fetch_data(simulate_item_ids=item_ids)
    print(f"Fetched {len(df_items)} items and {len(df_orders)} orders.")
